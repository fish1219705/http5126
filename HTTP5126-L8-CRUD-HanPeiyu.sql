...
--  1 
-- A 
CREATE TABLE customer (email VARCHAR(100),name VARCHAR(255) NOT NULL,PRIMARY KEY (email));
-- B 
...
-- C 
CREATE TABLE `order` (order_id INT AUTO_INCREMENT,email VARCHAR(100) NOT NULL,PRIMARY KEY(order_id),FOREIGN KEY(email) REFERENCES customer(email));
-- D
...
--  4 
-- A 
DELETE FROM order_product WHERE order_id = 1 OR order_id = 3;
-- B 
DELETE FROM `order` WHERE order_id = 1 OR order_id = 3;
